...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("im117_log.log", encoding="utf-8")
...
def rutas_autentificacion(app):
    ...
    def index():
        try:
            logging.info(
                f"Se accedió a la página de inicio de sesión - {CATALOGO_ERRORES['ErrorIM117_00']}")
            return render_template('index.html')
        except Exception as e:
            logging.error(f"ErrorIM117_01: {
                          CATALOGO_ERRORES['ErrorIM117_01']} Detalle: {e}")
    ...
    def login():
        if request.method == 'POST':
            ...
            logging.info(
                f"Intento de inicio de sesión para el usuario: {usuario}")
            ...
            try:
                ...
                logging.error(f"ErrorIM117_03: {
                              CATALOGO_ERRORES['ErrorIM117_03']} Detalle: {e}")
                return render_template('error.html', mensaje=f"ErrorIM117_03: {CATALOGO_ERRORES['ErrorIM117_03']}")

            if user and user['password'] == password:
                session['user'] = user
                logging.info(f"Inicio de sesión exitoso para el usuario: {usuario} con rol: {
                             user['rol']} - {CATALOGO_ERRORES['ErrorIM117_00']}")
                ...
                logging.warning(f"ErrorIM117_02: {
                                CATALOGO_ERRORES['ErrorIM117_02']} Usuario: {usuario}")
    ...
    def logout():
        ...
        if 'user' in session:
            ...
            logging.info(
                f"El usuario {usuario} cerró sesión - {CATALOGO_ERRORES['ErrorIM117_00']}")
            session.pop('user', None)
        else:
            logging.warning(f"ErrorIM117_04: {
                            CATALOGO_ERRORES['ErrorIM117_04']}")
